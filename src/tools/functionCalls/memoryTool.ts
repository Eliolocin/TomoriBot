...
export class MemoryTool extends BaseTool {
	...
	description = "Use this function when you identify a new, distinct piece of information, fact, preference, or instruction during the conversation that seems important to remember for future interactions. This helps you learn and adapt. Specify if the information is a general server-wide fact or something specific about a user. Avoid saving information that is already known or redundant.";
	...
	parameters: ToolParameterSchema = {
		...
		properties: {
			memory_content: {
				type: "string",
				description: "The specific piece of information, fact, or preference to remember. Be concise, clear, and ensure it's new information not already in your knowledge base.",
			},
			memory_scope: {
				type: "string",
				description: "Specify the scope of this memory. Use 'server_wide' for general information applicable to the whole server, or 'target_user' for information specific to a particular user.",
				enum: ["server_wide", "target_user"],
			},
			target_user_discord_id: {
				type: "string",
				description: "If memory_scope is 'target_user', provide the unique Discord ID of the user this memory pertains to (e.g., '123456789012345678'). This ID should be obtained from the user's information visible in the context.",
			},
			target_user_nickname: {
				type: "string",
				description: "If memory_scope is 'target_user', also provide the nickname of the user this memory pertains to, as you see them in the current conversation or their user profile information. This is used to confirm the target user alongside their Discord ID.",
			},
		},
	...
	async execute(args: Record<string, unknown>, context: ToolContext): Promise<ToolResult> {
		...
		const targetUserDiscordIdArg = args.target_user_discord_id as string | undefined;
		const targetUserNicknameArg = args.target_user_nickname as string | undefined;
		...
		const userRow = await loadUserRow(context.message?.author?.id || context.userId || "");
		
		if (!tomoriState || !userRow || !userRow.user_id || !tomoriState.server_id) {
			log.error("Critical state missing (tomoriState, userRow, or their IDs) before handling remember_this_fact");
			return {
				success: false,
		...
		if (memoryScopeArg === "server_wide") {
			...
			try {
				...
				if (dbResult) {
					...
					await sendStandardEmbed(context.channel, context.locale, {
						...
						descriptionVars: {
							memory_content: memoryContent.length > 200 ? `${memoryContent.substring(0, 197)}...` : memoryContent,
						},
			...
			if (typeof targetUserDiscordIdArg !== "string" || !targetUserDiscordIdArg.trim()) {
				return {
					success: false,
					error: "The 'target_user_discord_id' argument was missing or empty, which is required when 'memory_scope' is 'target_user'",
					data: {
						status: "memory_save_failed_invalid_args",
						scope: "target_user",
						reason: "The 'target_user_discord_id' argument was missing or empty, which is required when 'memory_scope' is 'target_user'"
					}
				};
			}
			...
			if (typeof targetUserNicknameArg !== "string" || !targetUserNicknameArg.trim()) {
				return {
					success: false,
					error: "The 'target_user_nickname' argument was missing or empty, which is required when 'memory_scope' is 'target_user'",
					data: {
						status: "memory_save_failed_invalid_args",
						scope: "target_user",
						reason: "The 'target_user_nickname' argument was missing or empty, which is required when 'memory_scope' is 'target_user'"
					}
				};
			}
			...
			try {
				...
				if (
					actualNicknameInDB.toLowerCase() !== targetUserNicknameArg.toLowerCase() &&
					actualNicknameInDB.toLowerCase() !== guildDisplayName
				) {
					log.warn(`Self-teach: Nickname mismatch for target user ${targetUserDiscordIdArg}. LLM provided: '${targetUserNicknameArg}', DB has: '${actualNicknameInDB}'.`);
					return {
						success: false,
				...
				if (dbResult) {
					log.success(`Tomori self-taught a personal memory for ${targetUserNicknameArg} (Discord ID: ${targetUserDiscordIdArg}, Internal ID: ${targetUserRow.user_id}): "${memoryContent}"`);
				...
				log.error(`Failed to save personal memory for ${targetUserNicknameArg} (Discord ID: ${targetUserDiscordIdArg}) via self-teach (DB error)`);
				return {
					success: false,
