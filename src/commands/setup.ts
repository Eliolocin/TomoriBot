...
export async function execute(
  ...
  try {
    ...
    providerSelectOptions.push({
      label: localizer(locale, "commands.setup.api_provider_custom_endpoint_label"),
      value: SETUP_CUSTOM_ENDPOINT_PROVIDER,
      description: localizer(locale, "commands.setup.api_provider_custom_endpoint_description"),
    });
    if (!isDMChannel) {
      providerSelectOptions.push({
        label: localizer(locale, "commands.setup.api_provider_user_byok_label"),
        value: SETUP_USER_BYOK_PROVIDER,
        description: localizer(locale, "commands.setup.api_provider_user_byok_description"),
      });
    }
    ...
    const humanizerSelectOptions: RadioGroupOption[] = [
      {
        label: localizer(locale, "commands.setup.humanizer_option_none_label"),
        value: "0",
        description: localizer(locale, "commands.setup.humanizer_option_none_desc"),
      },
      {
        label: localizer(locale, "commands.setup.humanizer_option_light_label"),
        value: "1",
        description: localizer(locale, "commands.setup.humanizer_option_light_desc"),
      },
      {
        label: localizer(locale, "commands.setup.humanizer_option_default_label"),
        value: "2",
        description: localizer(locale, "commands.setup.humanizer_option_default_desc"),
      },
      {
        label: localizer(locale, "commands.setup.humanizer_option_heavy_label"),
        value: "3",
        description: localizer(locale, "commands.setup.humanizer_option_heavy_desc"),
      },
    ];
    ...
    const modalResult = await promptWithRawModal(
      ...
      locale,
      {
        ...
        modalTitleKey: "commands.setup.modal_title",
        components: [
          {
            customId: "api_provider",
            labelKey: "commands.setup.api_provider_label",
            descriptionKey: "commands.setup.api_provider_description",
            placeholder: "commands.setup.api_provider_placeholder",
            required: true,
            options: providerSelectOptions,
          },
          {
            customId: "api_key",
            labelKey: "commands.setup.api_key_label",
            descriptionKey: "commands.setup.api_key_description",
            placeholder: "commands.setup.api_key_placeholder",
          ...
          },
          {
            ...
            labelKey: "commands.setup.preset_label",
            descriptionKey: "commands.setup.preset_description",
            placeholder: "commands.setup.preset_placeholder",
            required: true,
          ...
          },
          {
            ...
            labelKey: "commands.setup.humanizer_label",
            descriptionKey: "commands.setup.humanizer_description",
            required: true,
            options: humanizerSelectOptions,
          },
          {
            customId: "timezone_offset",
            labelKey: "commands.setup.timezone_label",
            descriptionKey: "commands.setup.timezone_description",
            style: TextInputStyle.Short,
            placeholder: "commands.setup.timezone_placeholder",
    ...
    try {
      ...
      if (!apiProvider || !presetName || !humanizerDegreeStr) {
        ...
        await replyInfoEmbed(modalSubmitInteraction, locale, {
          ...
          descriptionKey: "commands.setup.modal_values_missing",
          color: ColorCode.ERROR,
        });
        return;
      }
      ...
      if (!apiProvider || (!isUserByokSetup && !isCustomEndpointSetup && !normalizedProvider)) {
        await replyInfoEmbed(modalSubmitInteraction, locale, {
          titleKey: "general.errors.operation_failed_title",
          descriptionKey: "commands.setup.provider_invalid",
          color: ColorCode.ERROR,
        });
        return;
      }
      ...
      if (isUserByokSetup) {
        ...
        if (!apiKey || apiKey.length < 10) {
          await replyInfoEmbed(modalSubmitInteraction, locale, {
            titleKey: "general.errors.operation_failed_title",
            descriptionKey: "commands.setup.api_key_invalid",
            color: ColorCode.ERROR,
          });
          return;
        }
        ...
        await replyInfoEmbed(modalSubmitInteraction, locale, {
          titleKey: "commands.setup.api_key_validating",
          description: localizer(locale, "commands.setup.api_key_validating_description"),
        ...
        try {
          ...
          await replyInfoEmbed(modalSubmitInteraction, locale, {
            titleKey: "general.errors.operation_failed_title",
            descriptionKey: "commands.setup.api_key_invalid_api",
            color: ColorCode.ERROR,
          });
          return;
        }
      ...
      if (!selectedPresetOption) {
        await replyInfoEmbed(modalSubmitInteraction, locale, {
          titleKey: "general.errors.operation_failed_title",
          descriptionKey: "commands.setup.preset_invalid",
          descriptionVars: {
      ...
      if (!presetRow) {
        await replyInfoEmbed(modalSubmitInteraction, locale, {
          titleKey: "general.errors.operation_failed_title",
          descriptionKey: "commands.setup.preset_not_found",
          color: ColorCode.ERROR,
        });
        return;
      }
      ...
      if (Number.isNaN(parsedHumanizer)) {
        await replyInfoEmbed(modalSubmitInteraction, locale, {
          titleKey: "general.errors.operation_failed_title",
          descriptionKey: "commands.setup.humanizer_invalid",
          color: ColorCode.ERROR,
        });
        return;
      }
      ...
      if (timezoneOffsetStr?.trim()) {
        ...
        if (Number.isNaN(parsedOffset)) {
          await replyInfoEmbed(modalSubmitInteraction, locale, {
            titleKey: "general.errors.operation_failed_title",
            descriptionKey: "commands.setup.timezone_invalid_format",
            descriptionVars: {
        ...
        if (parsedOffset < -12 || parsedOffset > 14) {
          await replyInfoEmbed(modalSubmitInteraction, locale, {
            titleKey: "general.errors.operation_failed_title",
            descriptionKey: "commands.setup.timezone_out_of_range",
            descriptionVars: {
      ...
      try {
        ...
        await replyInfoEmbed(modalSubmitInteraction, locale, {
          titleKey: "general.errors.operation_failed_title",
          descriptionKey: "commands.setup.config_invalid",
          color: ColorCode.ERROR,
        });
        return;
      }
      ...
      try {
        ...
        await replyInfoEmbed(modalSubmitInteraction, locale, {
          titleKey: "general.errors.operation_failed_title",
          descriptionKey: "commands.setup.setup_failed_description",
          color: ColorCode.ERROR,
        });
        return;
      }
      ...
      if (isDMChannel) {
        successFields.push({
          nameKey: "commands.setup.dm_context_explanation_title",
          value: localizer(locale, "commands.setup.dm_context_explanation"),
        });
      }

      successFields.push({
        nameKey: "commands.setup.next_steps_title",
        value: localizer(
          locale,
          isDMChannel ? "commands.setup.next_steps_value_dm" : "commands.setup.next_steps_value",
        ),
      });

      successFields.push({
        nameKey: "commands.setup.learn_more_title",
        value: localizer(locale, "commands.setup.learn_more_value", {
      ...
      if (isCustomEndpointSetup) {
        ...
        headsUpNotes.push({
          label: localizer(locale, "commands.setup.custom_endpoint_bootstrap_field"),
          detail: localizer(locale, "commands.setup.custom_endpoint_bootstrap_value", {
      ...
      const headsUpEmbed =
        headsUpNotes.length > 0
          ? new EmbedBuilder()
              ...
              .setTitle(localizer(locale, "commands.setup.heads_up_title"))
              .setDescription(headsUpNotes.map((note) => `- **${note.label}**\n  - ${note.detail}`).join("\n"))
          : null;

      const successDescriptionKey = isUserByokSetup
        ? isDMChannel
          ? "commands.setup.success_desc_dm"
          : "commands.setup.success_desc_byok"
        : isCustomEndpointSetup
          ? "commands.setup.success_desc_custom_endpoint"
          : configuredModelName
            ? isDMChannel
              ? "commands.setup.success_desc_dm_with_model"
              : "commands.setup.success_desc_with_model"
            : isDMChannel
              ? "commands.setup.success_desc_dm"
              : "commands.setup.success_desc";
