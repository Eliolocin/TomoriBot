...
- Limit auto-trigger channels`,
    ...
    visualize: {
      ...
      cooldown_active: `This server's managers have configured a cooldown. Please wait **{seconds}** seconds before using \`/generate image\` in Auto mode again. This cooldown is shared with message triggers and other manual commands.`,
      channel_not_whitelisted: `This server has whitelist restrictions active. \`/generate image\` in Auto mode can only be used in whitelisted channels by members with whitelisted roles, and only with personas allowed in this channel.`,
      persona_access_blocked: `Your current whitelist permissions and personal spotlight settings do not leave any personas available for \`/generate image\` in Auto mode in this channel.`,
