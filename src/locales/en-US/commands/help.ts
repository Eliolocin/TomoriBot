...
**3.** Run {configMcpAdd} and choose **+ Add MCP**. In the form, paste the connection URL into **URL**, paste your Smithery API key into **Auth Token**, and choose the required **Server Type**. **General Purpose** is selected by default.
...
Your auth token is encrypted at rest and never shown in plain text after saving.`,
      online_summary_description: `Open {configMcpAdd} and choose **+ Add MCP** to register a publicly reachable HTTPS MCP server. Auth tokens are encrypted after saving; check the docs for provider-specific connection URL notes.`,
...
If you are running your own instance, point the URL to your local server (e.g. \`http://localhost:3000/sse\`). No auth token is needed for local servers.`,
      ...
      removing_description: `Open {configMcpAdd}, select the server, and choose **Remove**. Confirming immediately disconnects the server and frees a slot for a new one.`,
