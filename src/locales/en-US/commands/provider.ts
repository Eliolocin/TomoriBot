export default {
  provider: {
    ...
    remove: {
      ...
      active_provider_note: `**{provider}** is your active **text model** provider and cannot be removed while in use. Switch to a different provider with \`/config\` > Models > Switch Models first.`,
...
{reassignments}`,
    ...
    "api-key": {
      ...
      set: {
        ...
        api_key_description: `This key will be securely stored. Open /help to Setup, then Step 1: Get an API Key, for instructions.`,
        api_key_description_with_custom: `API Key, or OpenAI endpoint URL if using Custom (e.g., http://localhost:11434/v1)`,
        api_key_placeholder: `Do NOT share this key with anyone`,
        bearer_token_label: `Bearer Token (Optional)`,
        bearer_token_description: `Auth token for Custom endpoints. Sent as Authorization: Bearer header.`,
