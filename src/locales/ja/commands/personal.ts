...
\`/personal privacy\`を使用していつでも変更できます。`,
    ...
    nickname: {
      ...
      nickname_description: `空欄の場合は現在のDiscord表示名を使用します。`,
      prefix_mode_label: `接頭辞の動作`,
      suffix_mode_label: `接尾辞の動作`,
      prefix_label: `カスタム接頭辞`,
      suffix_label: `カスタム接尾辞`,
      affix_text_description: `「カスタム」を選択した場合のみ使用します。`,
      inherit_option: `継承`,
      inherit_description: `次の全体設定またはペルソナ既定値を使用します。`,
      none_option: `なし`,
      none_description: `継承される値を明示的に無効化します。`,
      custom_option: `カスタム`,
      custom_description: `下に入力した空でない文字列を使用します。`,
      invalid_affix_title: `カスタム値が必要です`,
      invalid_affix_description: `「カスタム」を選択した接頭辞・接尾辞には空でない文字列を入力してください。`,
...
**警告:** 現在、このサーバーではパーソナライズが無効になっているため、このニックネームはここでは使用しません。パーソナライズが有効になっている他のサーバーでは使用します。`,
    ...
    identity: {
      description: `任意のアイデンティティ情報と呼び方の好みを設定します。`,
      modal_title: `個人アイデンティティ`,
      gender_label: `性自認`,
      gender_description: `任意の自由記述です。空欄で消去します。`,
      pronouns_label: `代名詞`,
      pronouns_description: `例：she/her、どれでも、名前を使用。`,
      orientation_label: `性的指向`,
      orientation_description: `任意のプロフィール情報です。空欄で消去します。`,
      style_label: `希望する呼び方`,
      style_description: `ペルソナの呼称バリエーションを選択します。性自認から推測しません。`,
      style_unspecified: `指定なし`,
      style_masculine: `男性的`,
      style_feminine: `女性的`,
      style_neutral: `中立`,
      success_title: `アイデンティティ設定を更新しました`,
      success_description: `アイデンティティと呼び方の設定を更新しました。空欄の項目は消去されました。`,
    },
    impersonate: {
