  ...
  memory: {
    ...
    document: {
      ...
      view: {
        ...
        embedding_creds_missing_description: `編集したチャンクを再埋め込みするには、保存済みの埋め込みプロバイダー認証情報が必要です。\`/providers\` で設定してください。`,
        no_embedding_model_title: `埋め込みモデルが未設定です`,
        no_embedding_model_description: `埋め込みモデルが設定されていません。チャンクを編集する前に \`/config\` > Models > Switch Models で設定してください。`,
    ...
    history: {
      ...
      remove: {
        ...
        none_description: `このスコープには削除できる履歴ドキュメントがありません。\`/learn history\`で抽出してください。`,
...
**確認** を押して次に進みます。保存前に内容を確認・編集できます。`,
        ...
        no_embedding_model_title: `埋め込みモデルが未設定です`,
        no_embedding_model_description: `埋め込みモデルが設定されていません。記憶をベクトル化する前に \`/config\` > Models > Switch Models で設定してください。`,
        embedding_creds_missing_title: `埋め込み用認証情報が未設定です`,
        embedding_creds_missing_description: `この記憶をドキュメントとして埋め込むには、保存済みの埋め込みプロバイダー認証情報が必要です。\`/providers\` で設定してください。`,
        ...
        doc_limit_description: `**{persona_name}** には既に{current_count}件のドキュメントがあります（最大{max_allowed}件）。\`/memories\` で削除してから追加してください。`,
        ...
        partial_failure_description: `**{persona_name}** にドキュメント **{name}**（{chunk_count}チャンク）を作成しましたが、元のサーバー記憶を削除できませんでした。現在は両方が存在します。元の記憶を \`/memories\` で削除してください。新しいドキュメントを先に削除しない限り、再度ベクトル化しないでください。`,
