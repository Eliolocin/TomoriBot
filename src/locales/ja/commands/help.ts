...
**3.** {configMcpAdd} を実行して **+ MCPを追加** を選びます。フォームの**URL**に接続URLを、**認証トークン**にSmithery APIキーを貼り付け、必須の**サーバータイプ**を選びます。初期値は**汎用**です。
...
認証トークンは保存後に暗号化され、平文で表示されることはありません。`,
      online_summary_description: `{configMcpAdd} を開いて **MCPを追加** を選ぶと、公開された HTTPS MCP サーバーを登録できます。認証トークンは保存後に暗号化されます。接続URLの注意点はドキュメントを確認してください。`,
...
自己ホストの場合は、ローカルサーバーのURLを指定してください（例：\`http://localhost:3000/sse\`）。ローカルサーバーには認証トークンは不要です。`,
      ...
      removing_description: `{configMcpAdd} を開いてサーバーを選び、**削除**を選択します。確認すると即座に接続が切断され、新しいサーバーのスロットが解放されます。`,
