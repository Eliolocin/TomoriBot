export default {
  forget: {
    sampledialogue: {
      ...
      no_dialogues: `削除するサンプル対話が保存されていません。\`/config\` > Persona > Generalで追加してください。`,
    ...
    attribute: {
      ...
      no_attributes: `削除する人格属性がありません。\`/config\` > Persona > Generalで追加してください。`,
    ...
    document: {
      ...
      none_title: `文書がありません`,
      none_description: `このスコープには削除できる文書がありません。\`/memories\`で追加してください。`,
...
**警告:** 現在、このサーバーではパーソナライズが無効になっているため、この変更はここでの私の行動に影響しません。パーソナライズが有効になっている他のサーバーでは反映されます。`,
      ...
      server: {
        ...
        no_memories: `このサーバーにはサーバーの記憶が保存されていません。\`/memories\`で追加してください。`,
