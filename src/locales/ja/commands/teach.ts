export default {
  teach: {
    sampledialogue: {
      ...
      limit_exceeded_description: `このサーバーはサンプル対話の上限 {max_allowed} 個に達しました（現在 {current_count} 個）。新しいものを追加する前に、\`/config\` > Persona > Generalでいくつかのサンプル対話を削除してください。`,
...
> {bot_input}`,
    ...
    attribute: {
      ...
      limit_exceeded_description: `このサーバーは属性の上限 {max_allowed} 個に達しました（現在 {current_count} 個）。新しいものを追加する前に、\`/config\` > Persona > Generalでいくつかの属性を削除してください。`,
      batch_limit_exceeded_title: `一括インポートが上限超過`,
      batch_limit_exceeded_description: `インポートに {import_count} 枠必要ですが、上限 {max_allowed} / 現在 {current_count} です。{remove_count} 件削除してから再試行してください。`,
    ...
    document: {
      ...
      no_embedding_model_description: `このプロバイダーには埋め込みモデルが設定されていません。\`/config\` > Models > Switch Modelsで設定してください。`,
      ...
      limit_exceeded_description: `このスコープ（{scope}）は文書の上限 {max_allowed} 件に達しました（現在 {current_count} 件）。\`/memories\`で削除してください。`,
      invalid_file_title: `無効なファイル`,
...
**警告:** あなたは現在、このサーバーのパーソナライズ機能のブラックリスト対象のため、これらの記憶はここでは使用されません。`,
      ...
      server: {
        ...
        limit_exceeded_description: `このサーバーは記憶の上限 {max_allowed} 件に達しました（現在 {current_count} 件）。新しい記憶を追加する前に、\`/memories\`でいくつかの記憶を削除してください。`,
        batch_limit_exceeded_title: `一括インポートが上限超過`,
        batch_limit_exceeded_description: `インポートに {import_count} 枠必要ですが、上限 {max_allowed} / 現在 {current_count} です。{remove_count} 件削除してから再試行してください。`,
        content_too_long_title: `記憶の内容が長すぎます`,
        content_too_long_description: `記憶の内容が長すぎます。最大許容長は {max_length} 文字です。`,
