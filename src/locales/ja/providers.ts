...
{failure_list}`,
    ...
    tips: {
      ...
      switch_model_provider: `\`/model text\` または \`/providers\` で別のモデルやプロバイダーに切り替えてください。`,
      switch_model_provider_personal: `\`/personal provider model-text\` で切り替えるか、\`/personal providers\` で **テキスト** をオフにするとこのサーバーのモデルを使用します。`,
...
選択されたモデルは有償モデルのトレーニングにデータを提供することを許可する必要がありますが、現在のOpenRouterアカウントのプライバシー設定によりブロックされています。`,
    ...
    nvidia: {
      ...
      "500_parameter_default_message": `NVIDIA はこのモデルで一部のリクエストパラメータを拒否しました。詳細に \`min_p\` などの未対応サンプラーパラメータが表示されている場合は、\`/model parameters\` で \`0\` に設定して無効にしてください。詳細に \`logit_bias\` が表示されている場合は、\`/model logit-bias remove\` で保存済みエントリを削除してください。`,
      unknown_default_message: `予期しないエラーが発生しました`,
    },
    self_teach: {
