...
This document tracks the progress of the immediate focus of the project. Should be updated everytime progress on the work is done.

## TomoriBot LLM Provider Abstraction Refactor

Currently, we are in the process of refactoring TomoriBot from a tightly-coupled Gemini implementation to a modular provider architecture that supports multiple LLM providers.
...
This section outlines the plan to modularize TomoriBot's streaming architecture, extracting ~600 lines of universal Discord logic from the Google-specific `streamGeminiToDiscord` function into a reusable `StreamOrchestrator` that can work with any LLM provider.
