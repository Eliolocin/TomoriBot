...
| #5 `/status` split with retained `/tool status` compatibility | `src/commands/status.ts`; compatibility leaf `src/commands/tool/status.ts`; `src/utils/metrics/status/command.ts` | Deleted `statusCommandMetrics.ts` and `status/commandImplementation.ts`; owned modules under `src/utils/metrics/status/` are all <600 lines | Complete | Canonical status command coordination, page builders, and redaction-aware formatters, with the compatibility leaf using the same coordinator |
