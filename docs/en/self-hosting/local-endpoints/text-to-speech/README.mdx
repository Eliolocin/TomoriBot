...
3. Select the saved endpoint, choose **Add or Edit a Model**, and add its exact model code.

Adding the model makes the endpoint active immediately. Use `/model speech` later only when switching between registered TTS endpoints.
