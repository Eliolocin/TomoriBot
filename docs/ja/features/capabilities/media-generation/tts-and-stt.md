...
ラッパーサーバーを起動し、`/providers`で登録し、`/providers`で選択し、`/config`でサンプルをアップロードし、最後に`/speech voice-assign`で割り当てます。どのような音声形式でも受け入れられます（モノラルのWAVに自動変換されます）。BGMのない10〜20秒のクリップが最適です。
