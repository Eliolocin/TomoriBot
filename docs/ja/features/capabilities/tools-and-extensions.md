...
すべての項目を検証した後、1つのトランザクションで保存します。制限の強いプライバシー設定では追加と変更を拒否しますが、消去は許可します。ペルソナ全体の呼びかけ語は編集できません。`/capabilities manage`の「ユーザー情報の自動更新」は、ツールの表示と古い直接呼び出しの両方を防御します。無効でも`/personal config`と`/personal config`は使用できます。
