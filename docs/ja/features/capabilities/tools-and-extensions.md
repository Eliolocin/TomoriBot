...
参加者コンテキストでは各ユーザーの接頭辞と接尾辞がニックネームとは別に示されるため、称号をやめる依頼はニックネームの書き換えではなく接辞の変更として解決されます。接辞を`none`にすると、どの層に由来する値でも抑制されます。ペルソナ別の上書きが全体設定の`none`より優先される場合は、同じトランザクションでその上書きも消去するため、明示的な抑制が無視されることはありません。`inherit`はそのまま扱います。下位の層に戻ることがその意味だからです。

すでに解決済みの接辞を含むニックネームが送信された場合は、解決済みの値と照合して重複部分を取り除きます。境界を推測するためにニックネームを空白で分割することはありません。更新が成功するたびに結果の呼び名を報告するため、呼び方の変更は同じターン内で反映されます。

すべての項目を検証した後、1つのトランザクションで保存します。制限の強いプライバシー設定では追加と変更を拒否しますが、消去は許可します。ペルソナスコープは常に現在のペルソナ系統を意味し、ペルソナ全体の呼びかけ語は編集できません。`/capabilities manage`の「ユーザー情報の自動更新」は、ツールの表示と古い直接呼び出しの両方を防御します。無効でも`/personal profile about`と`/personal profile nickname`は使用できます。
