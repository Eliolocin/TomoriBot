...
組み込みの`update_user_info`ツールは、登録ユーザーのニックネーム、接頭辞、接尾辞、性自認、代名詞、性的指向、希望する呼び方、数値UTCオフセットを変更する明示的な依頼を処理します。他の個人向けツールと同じ、衝突を考慮した名前・別名・メンション・Discord IDの解決を使います。対象を省略すると会話を開始した人自身になり、`all`や`everyone`がワイルドカードになることはありません。

すべての項目を検証した後、1つのトランザクションで保存します。制限の強いプライバシー設定では追加と変更を拒否しますが、消去は許可します。ペルソナスコープは常に現在のペルソナ系統を意味し、ペルソナ全体の呼びかけ語は編集できません。`/capabilities manage`の「ユーザー情報の自動更新」は、ツールの表示と古い直接呼び出しの両方を防御します。無効でも`/personal identity`と`/personal nickname`は使用できます。
