...
ツールの使用は、有効なプロバイダーやモデルがツール呼び出しをサポートしているかどうかに依存します。また、多くのツールは機能フラグ（`/config` > Permissionsの切り替え）、Discordの権限、モデルの機能、またはオプションのAPIキーによって制限されています。
...
3. `/config`を実行して **+ MCPを追加** を選び、**URL**に接続URLを、**Auth Token**にSmitheryキーを貼り付け、必須の**サーバータイプ**を選びます。初期値は**汎用**です。

サーバーが認証を必要としない場合は、**Auth Token**を空白のままにしてください。認証トークンは保存時に暗号化され、二度と表示されません。`/config`では設定状態の確認、有効化・無効化、確認付きの削除を行えます。削除すると即座に切断され、スロットが解放されます。

保存済みの各行には、最後に成功した検出で得られたツール名も上限付きで表示されます。**検出なし**はツールが0件だった既知の結果、**検出状況不明**は従来の行または成功したスナップショットがまだない状態です。`/config`を開くだけでは保存済みメタデータを読み取るだけで、リモートサーバーには接続しません。
...
- `/personal config`：ユーザーが自身の設定を上書きします。
...
すべての項目を検証した後、1つのトランザクションで保存します。制限の強いプライバシー設定では追加と変更を拒否しますが、消去は許可します。ペルソナ全体の呼びかけ語は編集できません。`/config` > Permissionsの「ユーザー情報の自動更新」は、ツールの表示と古い直接呼び出しの両方を防御します。無効でも`/personal config`と`/personal config`は使用できます。
