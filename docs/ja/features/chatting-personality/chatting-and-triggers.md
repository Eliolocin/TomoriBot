...
サーバー管理者は`/config` > Persona > Triggersで、選択したペルソナのトリガーワードを追加または削除できます。一般メンバーはページを確認できますが、変更用の操作は無効になります。
...
- `/expressions initialize`を実行して、サーバーの絵文字とステッカーを登録し、トモリが正確に使用できるようにします。
...
- **現在の時刻**：`/config` > Behavior > Generalに基づくサーバーの現地時間と大まかな時間帯。さらに、`/personal config`を設定している場合は各ユーザーの現地時間。
...
プレゼンスはプライバシーによって保護されています。**Minimal**のプライバシーレベル（デフォルト：`/personal config`を参照）のユーザーで、かつbotでDiscordの*Guild Presences*インテントが有効になっている場合にのみ共有されます。プライバシー設定を高めているユーザーや、そのインテントを持たずに実行しているセルフホスト環境では、アクティビティはトモリに共有されません。
...
- `/personal config`：各ユーザーが自分用に上書きします。次の3つのモードがあります。
