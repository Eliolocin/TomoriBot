...
SillyTavernのプロンプトマネージャーのプリセットは、プロンプトの**レイアウト**を制御します。`/st-presets`でインポートし、`/st-presets`で有効なノードを確認し、`/st-presets`で通常のレイアウトに戻します。
...
- インポート済 ≠ 送信：`prompt_order`で無効になっているノードは、`/st-presets`で有効にするまでオフのままです。コメントのみや空のノードは送信されず、不明なマーカーはスキップされます。
